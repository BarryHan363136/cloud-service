package com.barry.cloud.platform.common.encrypt.hmac;
...
/**
 * Description: 定义加密方式  * MAC算法可选以下多种算法
 * 调用方式:
 * appkey=oMzNwvTcxg2qMSvxQAT5lxFgIuBWx6MMVp1DN+eNlR0=
 * //验证签名
 * String encryptKey = HMAC.encryptHMAC(JSONMapper.writeObjectAsString(requestInfo), appkey);
 * logger.info("请求签名为:"+signature+",验证签名为:"+encryptKey);
 * if (!signature.equals(encryptKey)){
 *     throw new RuntimeException("signature fail,Please check the request parameters");
 * }
 *
 * @author: hts
 * @created: 2018/09/02 11:40
 */
